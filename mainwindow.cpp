...
MainWindow::MainWindow(QWidget* parent) :
  ...
  for(int i = 0; i < game.SIZE; ++i)
    for(int j = 0; j < game.SIZE; ++j) {
      ...
      GameButton* gb = new GameButton(ui->centralWidget,
                                      i, j, value,
                                      ui->prototypeButton,
                                      game);
      game.setCell(i, j, gb);
      ...
      // после этого нужно проверить окончание
      // игры
      connect(gb, SIGNAL(clicked()),
              this, SLOT(checkGameOver()));
    }
}
  ...
  if (loadfile.open(QIODevice::ReadWrite )) {
    ...
    for (int i = 0; i < game.SIZE; i++) {
      ...
      for (int j = 0; j < game.SIZE; j++) {
        ...
        // после этого нужно проверить окончание
        // игры
        connect(gb, // Объект-источник сигнала
                SIGNAL(clicked()), // Какой сигнал?
                this, // Приёмник сигнала
                SLOT(checkGameOver()) // Какой слот?
               );
      }
    }
    loadfile.close();
